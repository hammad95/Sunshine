...
public class SunshineSyncAdapter extends AbstractThreadedSyncAdapter {
    ...
    public void onPerformSync(Account account, Bundle extras, String authority,
        ...
        try {
            ...
            if (buffer.length() == 0) {
                ...
                // Set location status to server down
            ...
            // to parse it. Set location status as server down.
            setLocationStatus(getContext(), LOCATION_STATUS_SERVER_DOWN);
        } catch (JSONException e) {
            Log.e(LOG_TAG, e.getMessage(), e);
            e.printStackTrace();
            setLocationStatus(getContext(), LOCATION_STATUS_SERVER_INVALID);    // Invalid server
        } finally {
    ...
    private void notifyWeather() {
        ...
        boolean notificationsEnabled = prefs.getBoolean(notificationsEnabledKey,
                Boolean.parseBoolean(context.getString(R.string.pref_enable_notifications_default)));

        // Create and display a notification if notifications are enabled
        if(notificationsEnabled) {
            ...
            if (System.currentTimeMillis() - lastSync >= DAY_IN_MILLIS) {
                ...
                if (cursor.moveToFirst()) {
                    ...
                    // Create an explicit intent for MainActivity.class
                    Intent mainActivityIntent = new Intent(context, MainActivity.class);

                    // Create a backstack to always navigate to home when user selects back
                    TaskStackBuilder stackBuilder = TaskStackBuilder.create(context);
                    // Add the backstack for the intent
                    stackBuilder.addParentStack(MainActivity.class);
                    // Add the intent that starts the activity to the top of the stack
                    stackBuilder.addNextIntent(mainActivityIntent);
                    // Get a PendingIntent for launching the activity constructed by stackBuilder
                    PendingIntent mainActivityPendingIntent =
                            stackBuilder.getPendingIntent(
                                    0,
                                    PendingIntent.FLAG_UPDATE_CURRENT
                            );
                    // Set the pending intent to mBuilder
    ...
    private void getWeatherDataFromJson(String forecastJsonStr,
        ...
        try {
            ...
            if ( cVVector.size() > 0 ) {
                ...
                inserted = mContentResolver.bulkInsert(WeatherContract.WeatherEntry.CONTENT_URI, cvArray);

                // Delete weather data from db that is more than one day old
                getContext().getContentResolver().delete(WeatherContract.WeatherEntry.CONTENT_URI,
                        WeatherContract.WeatherEntry.COLUMN_DATE + " <= ?",
                        new String[] {Long.toString(dayTime.setJulianDay(julianStartDay-1))});

                notifyWeather();    // Display notification after inserting new data into db
            }

            Log.d(LOG_TAG, "Sync Complete. " + inserted + " Inserted");
            // Set location status as successful
            setLocationStatus(getContext(), LOCATION_STATUS_OK);

        } catch (JSONException e) {
            Log.e(LOG_TAG, e.getMessage(), e);
            e.printStackTrace();
            // Set location status as invalid if JSON couldn't be parsed
